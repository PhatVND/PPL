...
def test_059():
    ...
    func stri(x: int) -> string { return "n=" + "x"; }

    func main() -> void {
        let r = (1 + 2) >> times2 >> stri;
    }
    """
    expected = "Static checking passed"
    assert Checker(source).check_from_source() == expected
...
def test_156():
    ...
    source = """
    func printi(s: string) -> void { return; }
    
    func main() -> void {  
        let x = printi("hi");
    }
    """
    assert Checker(source).check_from_source() == "Type Mismatch In Statement: VarDecl(x, FunctionCall(Identifier(printi), [StringLiteral('hi')]))"
...
def test_195():
    ...
    """
    assert Checker(source).check_from_source() == "Type Mismatch In Statement: Assignment(ArrayAccessLValue(ArrayAccessLValue(Identifier(matrix), IntegerLiteral(0)), IntegerLiteral(0)), FloatLiteral(3.14))"
...
def test_207():
    ...
    """
    assert Checker(source).check_from_source() == "Undeclared Function: a"
...
def test_225():
    ...
    """
    assert Checker(source).check_from_source() == "Type Mismatch In Statement: ExprStmt(FunctionCall(Identifier(printi), [StringLiteral('Hello, World!')]))"
