...
class Emitter:
    ...
    def emit_invoke_virtual(self, lexeme: str, in_, frame) -> str:
        ...
        typ = in_
        # Pop all params
        for _ in typ.param_types:  # <-- trước dùng typ.partype => check lại name đúng
            frame.pop()
        # Pop object ref
        frame.pop()
        # Nếu return type != void thì push 1
        if not isinstance(typ.return_type, VoidType):
            frame.push()
    ...
    def clear_buff(self) -> None:
        ...
        """
        self.buff.clear()
    def emit_println(self, in_type, frame) -> str:
        """
        Emit code to call java.io.PrintStream.println with the correct overload based on type.
        """
        # pop tham số ra khỏi stack sau khi println chạy
        # (nhưng trước đó phải giữ nguyên cho việc gọi)
        frame.pop()

        if type(in_type) is IntType:
            return self.jvm.emitINVOKEVIRTUAL("java/io/PrintStream/println", "(I)V")
        elif type(in_type) is FloatType:
            return self.jvm.emitINVOKEVIRTUAL("java/io/PrintStream/println", "(F)V")
        elif type(in_type) is BoolType:
            return self.jvm.emitINVOKEVIRTUAL("java/io/PrintStream/println", "(Z)V")
        elif type(in_type) is StringType:
    ...
    def emit_mulop(self, op, typ, frame):
        if op == '*':
            ...
            return self.jvm.emitIDIV() if isinstance(typ, IntType) else self.jvm.emitFDIV()
        else:
            raise Exception(f"Unsupported mulop: {op}")
    def emit_addop(self, op, typ, frame):
        if op == '+':
            return self.jvm.emitIADD() if isinstance(typ, IntType) else self.jvm.emitFADD()
        elif op == '-':
